    ...
    return OmegaConf.create(
        {
            ...
            "output": {"filename": "goal_diffusion_prior_results.txt"},
        }
    )


class GoalDiffusionPriorWorldPolicy(PixelEncoderMixin):
    def __init__(self, lewm_encoder, diffusion_prior, schedule, transform, action_mean, action_scale, device="cuda", execute_horizon=1, num_candidates=32):
    ...
    def act(self, info: Dict[str, Any]):
        ...
        candidates = self.schedule.sample(self.diffusion_prior, z_t, z_g, num_samples=self.num_candidates)
        action = self._select_best(z_t, z_g, candidates)
    ...
    def _select_best(self, z_t, z_g, candidates):
        ...
        cost = F.mse_loss(rollout, z_g.unsqueeze(1).expand_as(rollout), reduction="none").mean(dim=-1)
...
def run(cfg: DictConfig):
    ...
    policy = GoalDiffusionPriorWorldPolicy(
        lewm_encoder=lewm,
