...
from latent_subgoal_act.dp_latent_prior.common import ActionNormalizer
    ...
    return OmegaConf.create(
        {
            ...
            "output": {"filename": "dp_latent_prior_results.txt"},
        }
    )


class DPLatentWorldPolicy(PixelEncoderMixin):
    def __init__(
        self,
        lewm_encoder,
        model,
    ...
    def act(self, info: Dict[str, Any]):
        ...
        candidates = self.schedule.sample(self.model, z_t, z_g, num_samples=self.num_candidates)
        candidates = self.normalizer.unnormalize(candidates.reshape(-1, self.model.prediction_horizon, self.model.action_dim)).reshape(
            z_t.shape[0], self.num_candidates, self.model.prediction_horizon, self.model.action_dim
        )
...
def _load_policy(path, device):
    ...
    model = LatentTemporalUnet(**model_config)
    state = ckpt["ema_model"] if ckpt.get("ema_model") is not None else ckpt["model"]
    model.load_state_dict(state, strict=True)
    model.to(device).eval()
    schedule = DDPMScheduler(**ckpt["model_config"]["diffusion"], device=device)
    normalizer = ActionNormalizer.load(ckpt["normalizer"])
    return model, schedule, normalizer, ckpt
...
def run(cfg: DictConfig):
    ...
    model, schedule, normalizer, ckpt = _load_policy(policy_ckpt, device)
